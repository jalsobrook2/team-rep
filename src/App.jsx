...
export default function App(){
  // default to home so unauthenticated users land on the Home page
  ...
  const lastAuthToastRef = React.useRef(0)

  // When user becomes authenticated, ensure they land on the dashboard
  // and request the Dashboard to show the 'workers' panel (available workers).
  useEffect(()=>{
    if(user){
      setTab('dashboard')
      try{ window.dispatchEvent(new CustomEvent('dashboard-open', { detail: { panel: 'workers' } })) }catch(e){}
    }
  },[user])
  ...
  useEffect(()=>{
    ...
    function handleRequestTarget(target){
      ...
      // preserve intended target and notify the user, but avoid repeated toasts
    ...
    function onProfileNav(){
      if(user){ setTab('profile'); return }
      setPendingTarget('profile')
      try{
        const now = Date.now()
        if(!lastAuthToastRef.current || (now - lastAuthToastRef.current) > 3000){
          lastAuthToastRef.current = now
          toast('Please log in to access your profile — you will be returned after login', 'info')
        }
      }catch(e){}
      setTab('auth')
    }
    window.addEventListener('navigate-profile', onProfileNav)
    window.addEventListener('request-auth', onRequest)
    return ()=>{
      window.removeEventListener('navigate-dashboard', onNav)
      window.removeEventListener('navigate-profile', onProfileNav)
